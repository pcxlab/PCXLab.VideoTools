function Test-PCXSourceAudioSuitableForSynchronization {

    <#
    .SYNOPSIS
        Determines whether a media source's audio is suitable for synchronization.

    .DESCRIPTION
        Capability check that inspects a PCXLab.MediaSource and returns $true
        if its audio stream can be used for synchronization.
